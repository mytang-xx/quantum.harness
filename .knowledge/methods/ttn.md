...
- See `.knowledge/literature/magic/INDEX.md` for the TTN-coupled Pauli-Markov-sampling methodology reference (the source of the `O(χ⁴ log N)` per-update construction and the binary-tree coarse-graining recipe used in 2D Z_2 gauge / 2D Ising via Wegner duality).
