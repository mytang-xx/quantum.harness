...
This skill is for *Julia-specific configuration*. Cluster-side conventions (ssh, scheduler, partitions) live in `skills/using-slurm/profiles/<active>.toml` and are read here for `region` defaults and (when target is remote) the ssh alias and `repo_path_remote`.
...
|---|---|---|---|
| `--target` | `local` \| `remote:<alias>` | `local` | CLI; for remote, `<alias>` matches `skills/using-slurm/profiles/<active>.toml`'s `connection.ssh.alias` field; the skill reads `repo_path_remote` for where the project lives. |
...
2. **Configure mirror first** (only if `--region` resolves to a mirror): `scripts/setup-julia.sh mirror --region <name>`. This step runs BEFORE installing Julia even though the Jinguo guide numbers it "Step 2" — the mirror env var MUST be exported before the curl installer fetches juliaup. See [Mirror reference](#mirror-reference) for region defaults and URLs.
...
3. **Install Julia.** **Skip if** step 1's probe found a working `julia` whose `julia --version` matches `--version` (or matches the project's `julia_version` when `--version release`). Otherwise: `scripts/setup-julia.sh install [--region <name>] [--version X.Y.Z]`.
...
4. **Instantiate.** Local: `scripts/setup-julia.sh instantiate <project_dir>` (runs `Pkg.instantiate(); Pkg.precompile()` inside `<project_dir>`). Remote: same via `ssh <alias> 'cd <repo_path_remote> && scripts/setup-julia.sh instantiate julia-env'`.
5. **Verify**: `scripts/setup-julia.sh verify <project_dir> <package_name>` — three binary items, ALL MUST exit 0:
...
- Any `language = "julia"` stack contract in `skills/<stack>/stack.toml` — `/setup-julia` precedes the stack's install command.

**Data dependencies** (what `/setup-julia` reads):

- `skills/using-slurm/profiles/<active>.toml` — for the `region` default mirror and (for remote) the ssh alias and `repo_path_remote`.
