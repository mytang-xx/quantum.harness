...
- Stack contract: `skills/quspin/stack.toml`
...
- Route reason: official QuSpin code/example, custom `user_basis`, Python workflow requirement, or XDiag blocker. Otherwise recommend `/xdiag`.
...
- Diagnostics: basis dimension, sector label, term count, Hermiticity error, residuals, tiny dense brute-force comparison, and `/xdiag` comparison when feasible.
...
- First estimate paper size and the largest local-PC-in-15-min size. If the route is slower, memory-heavier, or less expressive than `/xdiag`, say so before asking the user to choose.
