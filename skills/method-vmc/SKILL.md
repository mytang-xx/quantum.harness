---
name: method-vmc
description: Use when a VMC, variational Monte Carlo, neural quantum state, NQS, NetKet, ansatz, sampler, or optimizer reproduction needs method-level route and tool selection.
---

# Method VMC

VMC/NQS is the variational stochastic track. Use it to decide whether a paper target is an ansatz benchmark, variational energy, or neural-state training task, then invoke the right tool skills.
...
1. Use VMC/NQS when the paper's claim is about a variational ansatz, training curve, energy benchmark, energy variance, or sign-problem regime where QMC is blocked.
...
- **Variance vs energy**: a low energy with high variance is suspicious. Report both.
