...
- **Output** — which figure(s)/panel(s) (one computation can feed several); and **per figure**: the observable plotted (y-axis) + normalization + which states it uses; the x-axis (parameter swept, range, spacing); what we expect to see and what would count as reproduced. **When the scope options carry materially different costs** (e.g. one panel runs at small size on a laptop while another needs a large lattice / GPU / cluster), the figure-scope question *is* a cost decision: give each option an order-of-magnitude **wall-time band pulled forward from the `/method-*` cost model** — the firm work count (flips / sweeps / matvecs) ÷ a stated assumed rate, with the rate named as the pre-probe unknown — so the choice is never cost-blind. Never present scope options whose cost spans orders of magnitude with only vague hints ("laptop-feasible", "wants a cluster") and no numbers. The detailed per-run-point cost table still comes in step 4; this is only the coarse band needed to choose scope.
- **Problem-required extras** — confirm what this target needs before any method is chosen. A finite-temperature target needs the temperature / β grid; a stochastic target needs the measured estimator and sampling target; a tensor-network target may need the requested bond-dimension range only if the paper makes it part of the reproduced object. Method-specific knobs are named as open decisions, not configured here.

Once the target figure/panel is identified, **show the user that paper figure** (see *Showing figures and opening the report*).

**Close step 0 with the setup card**, then pause and confirm before any method, software, or method-parameter question. The card is a reader-facing orientation card, not a saved artifact:

| Row | Content |
|---|---|
| Objective | The exact figure/panel or result being reproduced, and what "reproduced" means. |
| Problem | Plain-English model/system introduction plus the Hamiltonian, algorithmic object, or benchmark definition if known. |
| Fixed by source | Couplings, lattice, boundary, size, sector, initial/state selection, observable, normalization, and conventions fixed by the paper or handoff. |
| Open decisions | What still needs user choice next: usually method, software, scale, compute placement, or method-specific knobs. |

Present the card and ask `1. Confirm`, `2. Show source`, `3. Override/correct`. Do not proceed to steps 1–3 until the user confirms the setup card. If a method route is obvious from the paper or track, mention it after the card as the next decision, not as a problem-setup fact.
...
2. **Training handoff / Track README** — only when `/beginner-training` Track 2 selected the target; track-scoped objective context, not target selection.
3. **`/method-*` skill** — `/method-ed`, `/method-mps`, `/method-peps`, `/method-qmc`, `/method-ltrg`, `/method-mcrg`, `/method-vmc`, `/method-qcs`, `/method-mf`, `/method-polyopt`. Source for **step 1** (which method suits the target) and **step 2** (which tool the method routes to), plus **step 3 method setup** (the conceptual knobs and tricks).
...
> **When the figure needs a probe the package doesn't expose** — a per-iteration convergence trajectory, a custom gradient norm, a wall-time-vs-error curve — first reach for the package's **public iteration hook** (a callback / `finalize` / an iterator that yields each step); these read internal state without forking the algorithm and are performance-neutral. Only if no such hook exists, **dev the package** (`Pkg.develop` in Julia, `pip install -e` a clone in Python) and add the probe to the source *non-invasively* — instrument, don't reimplement the solver. Either way, flag the extra effort in the tool choice and confirm it: a custom probe is a place the reproduction can silently diverge from the paper.
...
- **Confirm compute device and estimate resource {survey}.** Read the method skill first to fix the route, then the selected `/using-*` skill's `Time estimate` before any scale question; combine with the method card's cost scaling (e.g. CPMC's ~size³ wall and `>1e11` long-run heuristic, LTRG's O(D⁶·Dc³) per step). Fill the cost table with one row per run point. If one tiny timing probe would materially improve the estimate, gate it explicitly before running it: it measures only a rate (matvec / sweep / sample / layer), yields no scientific result, is discarded, and exists for **better runtime estimate**. Skipping means the plan uses a rough estimate. No other compute before Approve.
...
#### When the figure is a performance / wall-time benchmark

A figure whose axis *is* cost — wall time (or iteration count) vs error, convergence-speed of competing algorithms, throughput vs size — is reproducible only if the hardware and the timing method are pinned. Two extra rules apply on top of the cost step, both **surfaced and confirmed with the user, never silently defaulted**:

- **Parallelism is an explicit, confirmed knob — device-aware.** Wall time is meaningless without the hardware it ran on, and a parallel speedup that differs between two software stacks (e.g. Julia BLAS vs numpy) will distort a *cross-method* comparison. So ask before running: on **CPU**, the core count and thread count (BLAS/OpenMP threads, `JULIA_NUM_THREADS` / `OMP_NUM_THREADS` and the numpy thread env); on **GPU**, the number and model of identical GPUs. Give the user the trade-off — serial 1-thread is the cleanest apples-to-apples (especially at small bond dimension / matrix size, where threading barely helps and adds cross-stack variance), a fixed *N* threads is realistic throughput — and run all competing methods under the **same** setting, serially. Record the chosen count as a `params` row so the report carries it.
- **Time only the algorithm — exclude one-time setup and the measurement itself.** Start the clock after, and stop it before, everything that is not the algorithm under test: compilation / JIT (Julia, JAX — run a **warm-up pass on a tiny system first** so the timed run is fully compiled, then reset the clock), MPO / gate construction, environment initialization, and the per-checkpoint measurement cost (the energy or gradient norm you log each step — subtract that logging time from the elapsed time, don't let it inflate the curve). The reported wall time is pure algorithm time; note in the plan that warm-up and setup were excluded.
...
1. **Run** the approved plan. The script lands at `tracks/<track>/solutions/<model>_<brief>.{jl|py}` and saves output (figures, data) under `tracks/<track>/results/<run>/`. Fix ordinary code breakage quietly only when the fix does not change the **problem, method, software route, observable, parameters, scale, or scope**; summarize the mechanical fix in the result note. Interrupt the user when a real choice is needed (e.g. the chosen tool genuinely can't express this target).
