...
1. **Intake (ask first).** Ask for the **expert name**, the **method / variant** (a single variant, or the **family the expert developed** — e.g. LTRG with its successors XTRG, tanTRG), and optionally a **publication source** (Scholar / arXiv / ORCID link or a paper list). If the name is ambiguous (several researchers share it), the method span is unclear (one variant vs. the family), or the source is missing, clarify in brainstorming style — one question at a time, never guess. Also fix the **output path** (method-group dir + variant abbreviation); when these diverge — e.g. distilling a family successor with no method skill of its own, such as XTRG, which routes to group `/method-ltrg` (dir `docs/ltrg/`) yet takes the abbreviation `xtrg`, giving `docs/ltrg/xtrg-weili-distillation.html` — present 2–3 path options and confirm, never guess.
...
Evidence names the **source paper and** the location (e.g. `LTRG'10 Fig. 4`, `XTRG'18 §III`) — the corpus spans several papers, so a bare "Fig. 4" is ambiguous. Mark the scannable keypoints with `==…==` (renders as a yellow highlight) and bold each trick name, so a human can skim the page. Keep `$…$` math **outside** `==…==` and `**…**` spans — a highlight or bold span cannot contain math and will silently fail to render. This is a property of Markdown-plus-math rendering, not of the report renderer alone: it breaks the same way in **your own chat messages** — the confirmation tables especially, where bolded trick names and expert names sit beside symbols — so never nest `$…$` inside `**…**` or `==…==` anywhere; put the symbol outside the span, or use plain words inside it (write `**fixed-N tanTRG**`, not `**fixed-$N$ tanTRG**`).
