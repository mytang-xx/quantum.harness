---
name: potts-clock
description: Use when the user is working on a `q`-state quantum Potts or quantum Clock ground-state problem — qudit Hilbert space with discrete `Z_q` symmetry, ferromagnet/paramagnet quantum phase transition, parafermion criticality, or a generic qudit-spin generalization of TFIM.
---
...
| Tiny cluster (`N ≲ 16` for `q = 3`), exact spectrum | ED | `knowledge-base/methods/ed.md` |
...
| Diagnostic | Action |
|---|---|
| `magic` | Hand off to `physics/magic`. Two-site Pauli-flip updates preserve `Z_q` symmetry; `L(ρ_AB)` is the recommended critical diagnostic via `methods/pauli-markov.md` and `methods/ttn.md`. Benchmark anchor: `ν_Potts = 5/6` from `knowledge-base/magic-benchmarks.md`. |
