...
//         progress/state.toml.
// Writes: site/lib/data.ts (regenerated each invocation),
//         <run-dir>/report_<run-id>_<YYYY-MM-DD>.html,
//         <run-dir>/report_latest.html (symlink).
//
// Pipeline: parse → materialize typed data → regenerate lib/data.ts
//           → next build (static export) → inline assets → write HTML.
//
...
// ─── write site/lib/data.ts ────────────────────────────────────────────────
async function writeDataModule(data) {
  ...
  await writeFile(join(SITE_DIR, 'lib', 'data.ts'), banner + body);
}
...
async function main() {
  ...
  stdout.write(`build: regenerating site/lib/data.ts\n`);
  await writeDataModule(data);
