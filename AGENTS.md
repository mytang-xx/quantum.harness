...
- No first-cell provenance. Per-cell run-spec overrides are allowed, but assembly must validate each manifest against the merged shared+cell settings and report settings as constant vs varying. Never summarize a parameter budget, method choice, sector, boundary condition, or error rule from the first completed manifest unless a manifest-consensus check has proved it is global.
...
- **Long chains / iterative computes must emit intermediate estimates, not just final values.** A Markov chain running 10⁶ steps without any in-chain print is a 3-hour blind spot — the user can't sanity-check whether the chain is converging, whether the acceptance ratio is reasonable, or whether the running mean has stabilized. Print a partial estimate every K steps (where K is chosen so the user sees ~10–50 updates over the run): e.g., `[step 100k/1M] running mean = …, partial c_L = …, acceptance = …, blocks = …`. Same applies to long DMRG sweeps (per-sweep energy + truncation error), VMC training (per-epoch energy + variance), TEBD evolution (per-Trotter-step energy + entanglement). The script's standard runner enforces this via a `progress_every` knob; method cards declare a sensible default.
- **Monitor before declaring success — don't fire-and-forget remote actions.** A "RUNNING" status / a 0 exit code from `ssh` is not success; only verified output is. After any non-trivial remote action, stay engaged through a *settle-time* before reporting "✓ done":
  ...
  - **Sbatch grid submission**: 1–3 min settle-time. `squeue` "RUNNING" alone is not success — tail at least one cell's log to confirm real compute (Stage 1 anchors / chain steps / DMRG sweeps) is actually happening. Catches early failures (PATH issues, wrong binary, missing modules, OOM-at-startup) before they multiply across the grid.
