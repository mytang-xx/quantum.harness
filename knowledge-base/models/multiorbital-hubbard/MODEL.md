...
- **Cross-method validation** (when feasible) — re-solve at smaller orbital count or with density-density-only interactions as a sanity check; use ED only after `knowledge-base/methods/ed/METHOD.md` is rebuilt. See AGENTS.md "Verification practice".
