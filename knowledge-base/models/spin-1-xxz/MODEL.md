...
| 1D ring (PBC) at large `L` with Pauli-string sampling | MPS Based Algorithm | `knowledge-base/methods/mps-based-algorithm.md` |
...
| Condition | Action |
|---|---|
| Question is about magic / SRE / long-range magic across the phase diagram | Run the calculation here; hand off to `knowledge-base/physics/magic/PHYSICS.md`. Default partition for criticality is `L(ρ_AB)` because full-state magic *misses* both Néel-Haldane and Haldane-large-`D` transitions. See `knowledge-base/magic-benchmarks.md` for reference behavior. |
...
| Diagnostic | Action |
|---|---|
| `magic` | Hand off to `knowledge-base/physics/magic/PHYSICS.md`. Default estimator: `L(ρ_AB)` because full-state `m_n` does not distinguish the Haldane transitions. Two-site Pauli updates preserve `S^z_total` (U(1)) and the lattice symmetry. The Haldane-phase plateau saturates near the single-qutrit-product upper bound `(2/3) log 4 ≈ 0.92` — see `knowledge-base/magic-benchmarks.md`. |
